...
SSChunk *chunk;
	...
	chunk->buffer = inChunk->buffer;
	chunk->length = inChunk->length;							// in 16-bit stereo samples
	chunk->userdata = userdata;
	chan->loops_left = loops;
	
	chunk->bytebuffer = (signed char *)inChunk->buffer;
	chunk->bytelength = inChunk->length * 2 * 2;		// in bytes
...
int SSPlayChunk(int c, SSChunk *inChunk, int userdata, int loops, void(*FinishedCB)(int, int))
{
	...
	return SSEnqueueChunk(c, inChunk, userdata, loops, FinishedCB);
}
